...
class DialogueID:
    ...
    def current_id_plus(self):
        if os.path.exists(self.json_filename):
            print(f'当前对话ID[{self.dialogue_id}]log文件已创建')
            if self.dialogue_id_has_used():
                print(f'当前对话ID[{self.dialogue_id}]使用过')
                self.current_id += 1
                self.get_dialogue_id()
                print(f'已修改对话ID为[{self.dialogue_id}]')
                self.json_filename = os.path.join('LogDialogue', 'DialogueContentLog', f"{self.dialogue_id}.json")
                with open(self.file_path, 'w', encoding='utf-8-sig') as file:
                    json.dump(self.current_id, file, ensure_ascii=False, indent=4)
                self.current_id_plus()
            else:
                print(f'当前对话ID[{self.dialogue_id}]未使用过，不改变ID')
        else:
            print(f'当前对话ID[{self.dialogue_id}]log文件不存在')
    ...
    def init_currentID_by_json(self):
        ...
        if os.path.exists(self.file_path):
            with open(self.file_path, 'r', encoding='utf-8-sig') as file:
                ...
                print(f'已读取Json文件:[{self.file_path}]→当前文件ID:[{self.current_id}]')
        else:
            with open(self.file_path, 'w', encoding='utf-8-sig') as file:
                file.write(str(setting.start_number_dialogue_id))
                print(f'未找到文件,已创建Json文件:[{self.file_path}]→当前文件ID初始化:[{setting.start_number_dialogue_id}]')
    ...
    def make_dialogue_log_json(self):
        ...
        with open(self.json_filename, 'w', encoding='utf-8-sig') as file:
            json.dump(default_data,file, ensure_ascii=False, indent=4)
        print(f'当前对话ID[{self.dialogue_id}]log文件创建完毕')
    ...
    def dialogue_id_has_used(self):
        ...
        try:
            with open(self.json_filename, 'r', encoding='utf-8-sig') as file:
            ...
            print('damn我崩了!Json解析失败,你塞了个什么玩意在log目录里?')
            return True